...
function buttonclick(blogid,status,limit,page){
	...
	$(document).on('click', 'a#confirm_del_yes', function(eve){
		var web_address = 'http://'+window.location.hostname ;		
		$.ajax('http://www.kaffah.biz/reqpost/delete_transaction_byreseller/'+blogid, {
			dataType : 'json',
			type : 'POST',
...
function editorder(blogid,limit,page){
	$(document).on('click', 'a.editorder', function(eve){
		...
		$('#status option[value='+getVal('transaction_status',vall)+']').prop('selected', true);			
	...
	$(document).on('submit', '#formorder_edit', function(eve){
		...
		var status = null;	
		var web_address = 'http://'+window.location.hostname ;
		$.ajax('http://www.kaffah.biz/reqpost/updatetransaction_byreseller/'+blogid, {
			dataType : 'html',
			type : 'POST',
...
function gettransaction(blogid,status,limit,page){		
	...
	if(textsearch != null && textsearch != ''){
		url = 'http://www.kaffah.biz/reqpost/getorder_byreseller/'+blogid+'/'+limit+'/'+offset+'/s/'+textsearch;
	}
	else if(filterstatus != null && filterstatus != ''){
		url = 'http://www.kaffah.biz/reqpost/getorder_byreseller/'+blogid+'/'+limit+'/'+offset+'/f/'+filterstatus;
	}
	else{
		url = 'http://www.kaffah.biz/reqpost/getorder_byreseller/'+blogid+'/'+limit+'/'+offset;
	}
					...
					if(odd == 1){trclass = 'even';}
						...
						.append($('<tr>').attr('class', trclass)
							.append(
								...
								$('<td>').attr('class', 'invoice').html('<a id="'+trxid+'" class="editorder" title="'+trxid+'" href="#editorder" >'+trxid+'</a>'+
									'<div class="edit"><a id="'+trxid+'" class="editorder" title="'+trxid+'" href="#editorder" >Edit</a> | '+
									'<a id="'+trxid+'" class="delorder" title="'+trxid+'" href="#delorder">Hapus</a></div>'+
								...
								$('<td>').attr('class', 'trx').attr('id', 'transaction_id_'+trxid).html(
								...
								$('<td>').html(formatString(element.transaction_status)+'<br /><em class="red">'+formatString(element.transaction_temp_status)+'</em>')
	...
	return JSON.parse($.ajax({
	     ...
	     success: function(data) {
	        ...
	        $.each(data, function(index, element) {
	        	...
	        	$('#orderlist tbody tr td#transaction_id_'+trxid+' ul.transdetail').append('<li>'+formatString(element.name)+'<strong> x '+element.quantity+'</strong> </li>');
	        	$('#orderlist tbody tr td#transaction_id_'+trxid+' ul.transdetailfull').append('<li>'+formatString(element.name)+'<strong> x '+element.quantity+'</strong> : <strong>Rp  '+formatNumber(element.price)+'</strong>'+option+'</li>');
